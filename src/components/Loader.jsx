...
function AGSvg({ onComplete }) {
  return (
    ...
    >
      <svg viewBox="0 0 200 200" className="w-screen h-screen text-white" xmlns="http://www.w3.org/2000/svg">
    ...
    <nav className="fixed z-50 top-4 left-1/2 transform -translate-x-1/2 w-[90%] max-w-xl rounded-full bg-black/30 backdrop-blur-md p-2">
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <Tab tag="about" setPosition={setPosition}>About</Tab>
          <Tab tag="skills" setPosition={setPosition}>Skills</Tab>
          <Tab tag="contact" setPosition={setPosition}>Contact</Tab>
        </ul>
      </div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            {['home',"about", "skills", "contact"].map((tag) => (
              <li key={tag}>
                <button
                  onClick={() => {
                    scrollTo(tag);
...
const Tab = ({ children, setPosition, tag }) => {
  ...
  return (
    <li
      ...
      className="relative z-10 px-3 py-1.5 uppercase text-white mix-blend-difference md:px-5 rounded-full md:py-5 md:text-base cursor-pointer"
    >
      {children}
    </li>
  );
